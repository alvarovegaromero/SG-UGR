...
class MyScene extends THREE.Scene {
  constructor (myCanvas) {
    ...
    // Crear material para la linea
    var material_linea = new THREE.MeshLambertMaterial({color: 0x00ffff});

    //Crear línea visible del contorno usado en la revolucion
    var lineGeometry = new THREE.BufferGeometry();
    lineGeometry.setFromPoints (this.points);
    var line = new THREE.Line (lineGeometry, material_linea);

    line.rotation.y += 4.71239; //Lo colocamos sobre eje Z - 270º
    line.position.y+=2; //Lo ponemos como en el vídeo

    // Por último creamos el modelo.
    // El modelo puede incluir su parte de la interfaz gráfica de usuario. Le pasamos la referencia a
    // la gui y el texto bajo el que se agruparán los controles de la interfaz que añada el modelo.
    //this.model = new MyPolyhedron(this.gui, "Controles de la Caja");
    this.add (line); //añadimos la linea y el eje

    //La revolucion:

    this.ejespeon = new THREE.AxesHelper (5);
    this.peon = new MyRevolution(this.gui, "Peon", this.points);

    this.ejespeon.add(this.peon);
    this.ejespeon.position.set (7, 0, 0);

    this.add(this.ejespeon);
  }
